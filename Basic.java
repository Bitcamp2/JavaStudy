...
//		2. "유용한 클래스는 많다. 다양한 라이브러리를 활용한다."라는 StringBuffer가 있을 때 
//		사용자가 정수를 입력하고 위 StringBuffer를 
//		거꾸로 만든 StringBuffer의 사용자가 입력한 정수에 해당하는 인덱스의 문자를 출력하세요.

		// 사용자가 정수를 입력 할 수 있는 곳
		// 거꾸로 만들 것 d
		// 거꾸로 만든 StringBuffer를 배열에 넣기. d
		// 해당 인덱스 출력
		StringBuffer revSb = new StringBuffer("유용한 클래스는 많다. 다양한 라이브러리를 활용한다.");
		
		revSb.reverse(); // 거꾸로 만든 reverse 메소드
		
		System.out.println("0~29 숫자 중 하나를 고르세요."); // 입력값 안내문
		
		int num = sc.nextInt(); // 사용자 입력값을 담아주는 변수

		System.out.println(revSb.charAt(num)); // 정수에 해당하는 인덱스의 문자를 출력하기 위한 charAt() 메소드 ! // 이걸 실수함.
