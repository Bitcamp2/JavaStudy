...
//		2. 깊은 복사
//		깊은 복사는 heap 메모리 영역에 새로운 배열을 생성해서 전달한다.
//		고로, 서로 다른 배열을 바라 보게 하는 것이다.
		
//		2-1. clone메소드 사용
//		heap메모리 영역에 {1, 2, 3, 4, 5}라는 새로운 배열을 생성하고 그 주소를 전달한다.
...
//		다른 배열을 참조해서 사용한다. 고로 서로(원본과)에게 영향을 끼치지 않는다.
		copyArr1[0] = 100;
		copyArr2[0] = 1000;
		arr[0] = 10000;
